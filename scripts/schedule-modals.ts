        ...
        if (!cellState || !cellState.history || cellState.history.length === 0) {
            ...
            modalBody.innerHTML = `
                <ul class="history-list">
                    ${cellState.history
                    .map((entry) => {
                        const name = EmployeeManager.getEmployeeByUid(entry.userId ?? '')?.name || 'Nieznany';
                        const displayValue = entry.oldValue || '(pusty)';
                        const actualValue = entry.oldValue || '';
                        return `
                        <li class="history-item">
                            <div class="history-value">${escapeHTML(displayValue)}</div>
                            <div class="history-meta">
                                <span>${new Date(entry.timestamp).toLocaleString('pl-PL')}</span>
                                <span>przez: ${escapeHTML(name)}</span>
                            </div>
                            <button class="action-btn outline revert-btn" data-value="${escapeHTML(actualValue)}" title="Przywróć tę wartość"><i class="fas fa-undo"></i> Przywróć</button>
                        </li>
                    `;
                    })
                    .join('')}
