        ...
        if (!cellState || !cellState.history || cellState.history.length === 0) {
            ...
            modalBody.innerHTML = `
                <ul class="history-list">
                    ${cellState.history
                    .map((entry) => {
                        const escapedValue = escapeHTML(entry.oldValue);
                        const empName = EmployeeManager.getEmployeeByUid(entry.userId ?? '')?.name || 'Nieznany';
                        return `
                        <li class="history-item">
                            <div class="history-value">${escapedValue || '(pusty)'}</div>
                            <div class="history-meta">
                                <span>${new Date(entry.timestamp).toLocaleString('pl-PL')}</span>
                                <span>przez: ${escapeHTML(empName)}</span>
                            </div>
                            <button class="action-btn outline revert-btn" data-value="${escapedValue}" title="Przywróć tę wartość"><i class="fas fa-undo"></i> Przywróć</button>
                        </li>
                    `;
                    })
                    .join('')}
